...
class ProgressIndicator(QWidget):
    """Horizontal step indicator for wizard steps."""
    ...
    def __init__(self, parent: QWidget | None = None):
        ...
        self._step_labels = [
            ...
            tr("progress.step_kb"),
            tr("progress.step_opencode"),
            tr("progress.step_chat"),
        ]
        self._n_steps = len(self._step_labels)
        self.setMinimumHeight(60)
        self.setMaximumHeight(70)

    def set_current_step(self, step: int) -> None:
        """Set the current active step."""
        if 0 <= step < self._n_steps:
    ...
    def paintEvent(self, event) -> None:
        ...
        step_width = available_width / (n_steps - 1) if n_steps > 1 else 0
        ...
        for i in range(n_steps):
            ...
            if i < self._current_step:
                ...
                painter.drawLine(int(x - 4), circle_y, int(x - 1), circle_y + 3)
                painter.drawLine(int(x - 1), circle_y + 3, int(x + 5), circle_y - 3)
            else:
                painter.setPen(color_button_text)
                font_num = painter.font()
                font_num.setPointSize(9)
                font_num.setBold(True)
                painter.setFont(font_num)
                painter.drawText(int(x - 4), circle_y + 4, str(i + 1))
                painter.setFont(font)
    ...
    def mousePressEvent(self, event) -> None:
        ...
        margin = 60
        available_width = width - 2 * margin
        step_width = available_width / (self._n_steps - 1) if self._n_steps > 1 else 0
